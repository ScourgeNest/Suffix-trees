...
(define (cst-func suffixes)
  (cons (longest-common-prefix-of-list suffixes) (map (elim-prefix (length (longest-common-prefix-of-list suffixes))) suffixes)))
...
(define (not-null? list)
  (if (not (null? list))
      #t
      #f))

(define (get-ch-lambda words)
  (lambda (ch)
    (get-ch-words words ch)))

(define (extract-label labeling-func alphabet)
  (lambda (suffixes)
  (if (null? suffixes)
      '()
      (cons (car (labeling-func suffixes)) (suffixes->st labeling-func (cdr (labeling-func suffixes)) alphabet)))))

(define (suffixes->st labeling-func suffixes alphabet)
  (if (null? suffixes)
      '()
      (map (extract-label labeling-func alphabet) (filter not-null? (map (get-ch-lambda suffixes) alphabet)))))
...
(define (add-$ word)
  (if (null? word)
      '(#\$)
      (append (list (car word)) (add-$ (cdr word)))))

(define text->st
  (lambda (text)
    (lambda (labeling-func)
      (suffixes->st labeling-func (append (map add-$ (get-suffixes text)) (list '(#\$))) (sort (remove-duplicates (car (map add-$ (get-suffixes text)))) char<?)))))
